// cutoff.js — the Cutoff: one command that stops every Gate from passing
// anything, instantly, no matter what tier it's running at. Persisted to
// disk so it is honored across separate process invocations, not just
...
export function engage(cutoffPath, reason = 'manual cutoff') {
  writeJSONAtomic(cutoffPath, { engaged: true, engagedAt: new Date().toISOString(), reason });
}
